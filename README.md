Hi there, I'm Abhishek 
Technology executive focused on building AI-first digital platforms, enterprise-scale data ecosystems, and modern engineering organizations for regulated industries.
Currently leading Digital, Data & AI Platforms at India’s largest health insurer, driving transformation programs that power 65%+ of business revenue and deliver measurable impact through AI, automation, cloud-native platforms, and large-scale modernization initiatives.

🚀 **What I Work On**
Enterprise AI & Digital Transformation
Insurance & FinTech Platforms
Data & Decisioning Platforms
Microservices & API Ecosystems
Cloud-Native Architecture (AWS/Kubernetes)
AI-led Claims, Fraud & Automation
Platform Engineering & SRE
Enterprise Architecture & Governance

💡 **Key Highlights**
Built and scaled a 140+ member Technology Capability Center
Delivered $150M+ incremental business value through digital interventions
Led AI-driven claims adjudication reducing turnaround time by 30%
Built enterprise data & AI platforms processing billions of records
Modernized large-scale digital ecosystems across insurance and telecom domains
Led cloud-native API platforms handling 1000+ TPS at enterprise scale

🛠 **Tech & Architecture**
AI Platforms • AWS • Kubernetes • Microservices • Data Platforms • Hadoop • Kafka • Spark • Python • DevOps • SRE • Enterprise Architecture

📜 **Certifications**
AWS Certified Solutions Architect – Associate
TOGAF Certified Architect
PMP – Project Management Professional
IBM Certified Architect

🌍 **Interests**
AI Strategy • Enterprise Platforms • Agentic AI • Platform Engineering • Data Governance • Digital Insurance Ecosystems • Scalable Architecture

🤝 **Connect With Me**
LinkedIn: LinkedIn Profile
Email: abhishek.ambastha@gmail.com

<!--
**Abhishek126/Abhishek126** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->
